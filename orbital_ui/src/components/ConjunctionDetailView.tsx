...
// Mock data for the AreaChart (Pc History)
const mockHistoryData = Array.from({ length: 48 }, (_, i) => {
  const hour = 48 - i;
  const basePc = 1e-6;
  const spike = Math.exp(-Math.pow(hour - 12, 2) / 20) * 3e-4;
  const noise = Math.random() * 2e-5;
  return {
    time: `-${hour}h`,
    pc: basePc + spike + noise,
  };
});
...
export function ConjunctionDetailView({ event, onBack }: ConjunctionDetailViewProps) {
  ...
  const badgeColor = isUrgent ? "bg-red-500/20 text-red-400" : event.pc_band === "watch" ? "bg-amber-500/20 text-amber-400" : "bg-green-500/10 text-green-500";
    ...
    <div className="flex flex-col min-h-screen bg-[#060b14] text-slate-200 font-mono text-sm p-4 gap-[10px]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="text-slate-400 hover:text-white flex items-center gap-1 cursor-pointer transition-colors">
          ...
          <span className="font-bold text-slate-100">{event.obj1.name} ↔ {event.obj2.name}</span>
        </div>
        <div>
          <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-500 text-xs font-semibold tracking-wider">INVESTIGATING</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-[10px]">
        ...
        <div className="bg-[rgba(245,158,11,0.05)] border border-[rgba(245,158,11,0.1)] rounded-lg p-[10px] px-3 flex flex-col justify-between h-[80px]">
          ...
          <span className="text-xl text-amber-400">4.2 (Elevated)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-[10px] min-h-[220px]">
        ...
        <div className="bg-mission-panel border border-mission-border rounded-lg flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-4 relative min-h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={mockHistoryData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                ...
                <YAxis scale="log" domain={['auto', 'auto']} stroke="#3a5060" fontSize={10} tickFormatter={(val) => val.toExponential(0)} width={45} />
                <ReferenceLine y={1e-4} stroke="#ef4444" strokeDasharray="3 3" />
                <Area type="monotone" dataKey="pc" stroke="#ef4444" fillOpacity={1} fill="url(#colorPc)" isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-mission-panel border border-mission-border rounded-lg flex flex-col overflow-hidden">
          <div className="px-[14px] py-[8px] border-b border-mission-border bg-[rgba(255,255,255,0.02)]">
            <span>Space Weather Environment</span>
          </div>
          <div className="flex-1 p-5 flex flex-col justify-center gap-5">
            <div className="flex justify-between items-center border-b border-[rgba(255,255,255,0.06)] pb-2">
              ...
              <span className="text-amber-400 font-bold text-base">4.2</span>
            </div>
            <div className="flex justify-between items-center border-b border-[rgba(255,255,255,0.06)] pb-2">
              <span className="text-slate-400">Storm Level</span>
              <span className="text-amber-400 text-base">G1 (Minor)</span>
            </div>
            <div className="flex justify-between items-center border-b border-[rgba(255,255,255,0.06)] pb-2">
              <span className="text-slate-400">Atmospheric Drag Factor</span>
              <span className="text-red-400 text-base">+8.5%</span>
            </div>
            <div className="flex justify-between items-center pb-1">
              <span className="text-slate-400">Covariance Inflation</span>
              <span className="text-slate-200 text-base">1.2x</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 p-5 flex flex-col gap-6 justify-center">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded-full text-[9px] font-semibold tracking-wider ${badgeColor}`}>{badgeText}</span>
              ...
              <div className="flex justify-between text-xs pl-1 mt-1">
                <span className="text-slate-400">Type: <span className="text-slate-200 ml-1">{event.obj1.type}</span></span>
            ...
            <div className="flex flex-col gap-2 pt-5 border-t border-[rgba(255,255,255,0.06)]">
              ...
              <div className="flex justify-between text-xs pl-1 mt-1">
                <span className="text-slate-400">Type: <span className="text-slate-200 ml-1">{event.obj2.type}</span></span>
